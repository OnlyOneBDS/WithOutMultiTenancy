...
public class GetBrandRequestHandler : IRequestHandler<GetBrandRequest, BrandDto>
{
  ...
  {
    return await _repository.FirstOrDefaultAsync((ISpecification<Brand, BrandDto>)new BrandByIdSpec(request.Id), cancellationToken)
      ?? throw new NotFoundException(_t["Brand {0} Not Found.", request.Id]);
  }
}

public class BrandByIdSpec : SingleResultSpecification<Brand, BrandDto>
{
