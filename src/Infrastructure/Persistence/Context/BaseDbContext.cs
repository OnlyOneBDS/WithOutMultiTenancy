  ...
  IdentityUserToken<string>>
{
  ...
  protected BaseDbContext(DbContextOptions options, ICurrentUser currentUser, ISerializerService serializer, IOptions<DatabaseSettings> dbSettings, IEventPublisher events) : base(options)
  {
  ...
  public DbSet<Trail> AuditLogs => Set<Trail>();
  ...
  private List<AuditTrail> HandleAuditingBeforeSaveChanges(Guid userId)
  {
    ...
    foreach (var entry in ChangeTracker.Entries<IAuditableEntity>().Where(e => e.State is EntityState.Added or EntityState.Deleted or EntityState.Modified).ToList())
    {
    ...
    {
      AuditLogs.Add(auditEntry.ToAuditTrail());
    }
  ...
  private Task HandleAuditingAfterSaveChangesAsync(List<AuditTrail> trailEntries, CancellationToken cancellationToken = new())
  {
    ...
    foreach (var entry in trailEntries)
    {
      ...
      AuditLogs.Add(entry.ToAuditTrail());
    }
