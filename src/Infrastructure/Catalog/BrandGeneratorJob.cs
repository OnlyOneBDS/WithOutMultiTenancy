...
using WithOutMultiTenancy.Application.Catalog.Brands;
...
namespace WithOutMultiTenancy.Infrastructure.Catalog;
...
public class BrandGeneratorJob : IBrandGeneratorJob
{
  ...
  private async Task NotifyAsync(string message, int progress, CancellationToken cancellationToken)
  {
    ...
    await _notifications.SendToUserAsync(new JobNotification()
    {
