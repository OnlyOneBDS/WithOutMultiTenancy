...
using WithOutMultiTenancy.Application.Identity.Roles;
...
internal class RoleService : IRoleService
{
  ...
  {
    return (await _roleManager.Roles.ToListAsync(cancellationToken)).Adapt<List<RoleDto>>();
  }
  ...
  {
    return await _roleManager.FindByNameAsync(roleName) is ApplicationRole existingRole && existingRole.Id != excludeId;
  }

  public async Task<RoleDto> GetByIdAsync(string id)
  {
    return await _context.Roles.SingleOrDefaultAsync(x => x.Id == id) is { } role
      ? role.Adapt<RoleDto>()
